            ...
            if action == "1":
                ...
                if sj != '无事发生':  # 如果触发了具体事件
                    ...
                    if sj == '发现食物':
                        ...
                        if choice == '1':  # 悄悄查看分支
                            if random.randint(1, 2) == 1:  # 50% 发现遗弃背包
                                print(f"{GREEN}发现一个遗弃背包，获得罐头×1、水瓶×1！{RESET}")  # 成功提示
                                items['罐头']['拥有数量'] += 1
                                items['水瓶']['拥有数量'] += 1
                            else:  # 被野狗咬伤分支
                    ...
                    # 负面事件处理分支
                        ...
                        items['罐头']['拥有数量'] = max(0, items['罐头']['拥有数量'] - 1)  # 被抢走罐头
                        items['水瓶']['拥有数量'] = max(0, items['水瓶']['拥有数量'] - 1)  # 被抢走水瓶
                    elif sj == '受伤感染':
                        health -= 2  # 扣血
                        status['生病'] = True  # 设置生病
                        status['受伤'] = True  # 设置受伤
                    elif sj == '道具损坏':
                        breakable_items = [item for item in items if item not in ['猫咪', '电脑'] and items[item]['拥有数量'] > 0]  # 可损坏项列表
                        if breakable_items:  # 如果存在可损坏道具
                            broken_item = random.choice(breakable_items)  # 随机选择一件损坏
                            items[broken_item]['拥有数量'] -= 1  # 扣除该道具
                            print(f"{RED}道具损坏：{broken_item}×1{RESET}")  # 显示损坏提示
                    elif sj == '恶劣天气来袭':
                        ...
                        military_visited = True  # 标记军方到访
                        items['罐头']['拥有数量'] += 3  # 军方留下罐头
                        items['水瓶']['拥有数量'] += 3  # 军方留下水瓶
                    elif sj == '建筑物倒塌':
                        health -= 3  # 大幅度扣血
                        status['受伤'] = True  # 标记受伤
                    elif sj == '极端寒冷':
            ...
            # 3. 使用道具逻辑
                ...
                for i, (item_name, item_data) in enumerate(items.items()):  # 遍历道具并列出可用项
                    if item_data["拥有数量"] > 0 and item_name not in ['猫咪']:  # 排除猫咪（不可消耗）
                        usable_items.append((i+1, item_name, item_data))  # 添加索引与道具信息
                        print(f"{i+1}. {item_name}×{item_data['拥有数量']}")  # 打印道具选项
